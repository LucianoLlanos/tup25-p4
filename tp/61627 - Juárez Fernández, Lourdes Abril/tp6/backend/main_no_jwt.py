"""Backend simplificado para TP6 - Sin JWT, solo validación básica"""
...
def obtener_usuario_por_token(token: str, session: Session) -> Optional[Usuario]:
    sesion = session.exec(select(SesionUsuario).where(SesionUsuario.token == token)).first()
    if not sesion:
        return None
    
    if sesion.expires_at < int(time.time()):
...
def get_current_user(request: Request, session: Session = Depends(get_session)):
    ...
    user = obtener_usuario_por_token(token, session)
    if not user:
...
def register(user_data: dict, session: Session = Depends(get_session)):
    ...
    token = generar_token()
    expires_at = int(time.time()) + (24 * 3600)  # 24 horas
    sesion = SesionUsuario(usuario_id=new_user.id, token=token, expires_at=expires_at)
    session.add(sesion)
    session.commit()
...
def login(user_data: dict, session: Session = Depends(get_session)):
    ...
    token = generar_token()
    expires_at = int(time.time()) + (24 * 3600)  # 24 horas
    sesion = SesionUsuario(usuario_id=user.id, token=token, expires_at=expires_at)
    session.add(sesion)
    session.commit()
