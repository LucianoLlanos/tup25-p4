...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const data = await apiFetch("/iniciar-sesion", {
        method: "POST",
        body: JSON.stringify({ correo, password }),
      });
      login(data.access_token, data.usuario_nombre);
      router.push("/");
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col rounded-xl border bg-white p-6 shadow-sm">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="mt-2 w-full rounded-md bg-indigo-600 py-2 text-sm font-semibold text-white disabled:bg-slate-300"
          disabled={cargando}
        >
          Entrar
        </button>
